...
public class OpenTelemetryCollector implements UnitTestResource {
    ...
    private static class StallingRequestHandler {
        ...
        private final AtomicReference<HttpServerRequest> requestRef = new AtomicReference<>(null);
        ...
        private void handleTracesRequest(HttpServerRequest request) {
            ...
            if (requestRef.get() != null) {
                throwConcurrentGetTracesRequestNotAllowed();
            }

            if (helloEndpointCallTraced.get()) {
                respondWithTraces(request);
            } else {
                var oldReq = requestRef.getAndSet(request);
                if (oldReq != null) {
                    throwConcurrentGetTracesRequestNotAllowed();
                }
            }
        ...
        private void onReceivedTraces(GrpcMessage exportedTraces) {
            ...
            var req = requestRef.getAndSet(null);
            System.out.println("on received traces is req null " + req);
            if (req != null) {
                respondWithTraces(req);
            }
        }
        ...
        private void respondWithTraces(HttpServerRequest request) {
        ...
        private static void throwConcurrentGetTracesRequestNotAllowed() {
            // there shouldn't be a reason for next line to ever be invoked, if it is, design is wrong
            throw new IllegalStateException("Concurrency not implemented");
        }
    }
}
